...
h1
```
#### Plotting the American Option Prices for the Call using the Binomial Tree
```{r}
...
a$xAxis(categories = rownames(first))
...
### Plotting the Option Prices for the Put

```{r}

df_melted <- melt(option_chain_put,id.vars=c("Days_till_Expiry","Strike"), 
                  measure.vars = c("BinomialEuropean","BlackSholesMertonEuropean",
                                   "BinomialAmerican")) #,"Bid","Ask"
colnames(df_melted)[4]<-"Option_Price"
h1=hPlot(Option_Price ~ Strike, data =df_melted, type ='line',
