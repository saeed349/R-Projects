...
print(paste("European Call Price",round(BinomialTree(isCall=FALSE,K=100,Tm =1 ,S0 =100 ,sig = .2,N =200,r=.06),4)))
print(paste("European Call Price",round(BinomialTree(isCall=TRUE,isAmerican = TRUE,K=100,Tm =1 ,S0 =100 ,sig = .2,N=200,r=.06),4)))
print(paste("European Call Price",round(BinomialTree(isCall=FALSE,isAmerican = TRUE,K=100,Tm =1 ,S0 =100 ,sig = .2,N =200,r=.06),4)))
```
...
#### Plotting the European Option Prices for the Call using the Binomial Tree
...
library(rCharts)
library(reshape)
# df_melted <- melt(option_chain_call,id.vars=c("Days_till_Expiry","Strike"), 
#                   measure.vars = c("BinomialEuropean","BlackSholesMertonEuropean",
#                                    "BinomialAmerican","Bid","Ask")) #,
# df_melted <- melt(option_chain_call,id.vars=c("Days_till_Expiry","Strike"), 
#                   measure.vars = c("BinomialEuropean"))
# colnames(df_melted)[4]<-"Option_Price"
# h1=hPlot(Option_Price ~ Strike, data =df_melted, type ='line',
#           group = 'Days_till_Expiry')
# h1$chart(zoomType="xy")
# h1$xAxis(title = list(text = "Different Strikes"))
# # h1$yAxis(title='Option Price')
...
# h1
# 
# #Use this one 12:30 AM 8th March
...
a$yAxis(title = list(text = "Option Price"),replace = T)
a$data(first)
a
...
### Plotting the Option Prices for the Put Expiring in 26 days
...
### Plotting the Option Prices for the Put Expiring in 152 days
...
#### Plotting the Put Option Prices that are expiring in 5 days
Zoomin to see the difference in prices.
Also select legend to deactivate and activate individual series of option prices
```{r}
...
Secant_Binomial <- function(S, K, t, r, type, option_price
  ...
  theta=.00001
  print(type)
  fun.x1=BinomialTree(isCall=is.logical(type=="Call"),K=K,Tm =t ,S0 =S ,sig = x1,N =N,r=r)-option_price
  count=1
  start.time <- Sys.time()
  while(abs(fun.x1) > tolerance && count<max.iter) {
    x2=x1-theta
    # print(paste(isCall=is.logical(type=="Call"),K,t ,S ,x1,N,r))
    fun.x1=BinomialTree(isCall=is.logical(type=="Call"),K=K,Tm =t ,S0 =S ,sig = x1,N =N,r=r)-option_price
    fun.x2=BinomialTree(isCall=is.logical(type=="Call"),K=K,Tm =t ,S0 =S ,sig = x2,N =N,r=r)-option_price
    x1 <- x1- fun.x1/((fun.x1-fun.x2)/theta)   
    count <-count+1
  }
  end.time <- Sys.time()
...
option_chain_put_csv$premium<-(option_chain_put_csv$Bid+option_chain_put_csv$Ask)/2
#-----
df=ImpliedVol_Secant_Binomial(option_chain = option_chain_call_csv)
df=as.data.frame(df[1])
options.df=df[complete.cases(df$Implied_Volatility),]

# BinomialTree(isCall=is.logical('Call'=="Call"),K=130,Tm =t ,S0 =S ,sig = x2,N =N,r=r)
