...
for u=1:N
    for j=1:N
        psi(j) = BSCF(v(j)-(alpha+1)*i,S0,r,sigma,tau); %characterstic equation
        phi(j) = exp(-r*tau)*psi(j)/(alpha^2 + alpha - v(j)^2 + i*v(j)*(2*alpha+1)); %equation 6
%			d(j) = (alpha^2 + alpha - v(j)^2 + i*v(j)*(2*alpha+1));
        x(j) = exp(i*(b-s0)*v(j))*phi(j)*w(j);  % equation 24
        e(j) = exp(-i*2*pi/N*(j-1)*(u-1))*x(j); % equation 24
    end
