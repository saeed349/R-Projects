...
```{r}
print(paste("European Call Price=",Explicit2(isAmerican=FALSE,isCall=TRUE, K=100, Tm=1, S0=100, r=0.06, sig=0.25, N=as.integer(c[4]), div=0.03, dx=as.double(c[3]))))
# 11.01241
print(paste("European Put Price=",Explicit2(isAmerican=FALSE,isCall=FALSE, K=100, Tm=1, S0=100, r=0.06, sig=0.25, N=as.integer(c[4]), div=0.03, dx=as.double(c[3]))))
# 8.144172
print(paste("European Call Price=",Implicit2(isAmerican=FALSE,isCall=TRUE, K=100, Tm=1, S0=100, r=0.06, sig=0.25, N=as.integer(c[4]), div=0.03, dx=as.double(c[3]))$Price))
# 11.00983
print(paste("European Put Price=",Implicit2(isAmerican=FALSE,isCall=FALSE, K=100, Tm=1, S0=100, r=0.06, sig=0.25, N=as.integer(c[4]), div=0.03, dx=as.double(c[3]))$Price))
# 8.141817

print()
print(paste("Black Sholes Call Price=",BSM(type = 'c', K=100, t=1, S=100, r=0.06, sig=0.25,div=.03)))
print(paste("Black Sholes Put Price=",BSM(type = 'p', K=100, t=1, S=100, r=0.06, sig=0.25,div=.03)))
```
...
#Explicit Finite Difference method MODIFIED----
  # Explicit Implementation----
Explicit2 <- function(isAmerican, isCall, K, Tm,S0, r, sig, N, Nj=0, div, dx,returnGreeks=FALSE){
  # Finite Difference Method: i times, 2*i+1 final nodes
  # Precompute constants ----
  if(Nj!=0)
    returnGreeks=FALSE
...
#Implementation of Explicit----
print(paste("European Call Price=",Explicit2(isAmerican=FALSE,isCall=TRUE, K=100, Tm=1, S0=100, r=0.06, sig=0.2, N=3, div=0.03, dx=0.2,returnGreeks = TRUE))[1])
print(paste("European Put Price=",Explicit2(isAmerican=FALSE,isCall=FALSE, K=100, Tm=1, S0=100, r=0.06, sig=0.2, N=3, div=0.03, dx=0.2,returnGreeks = FALSE)))
#European put value is =5.6217
#European call value is =8.5455 (verified)
#Ameican put value = 6.0058 (verified)
...
#Implicit method MODIFIED----
  # Implicit Implementation----
Implicit2 = function(isAmerican,isCall, K, Tm,S0, r, sig, N, div, dx, Nj=0){
  # Implicit Finite Difference Method: i times, 2*i+1 final nodes
  # Precompute constants ----
  dt = Tm/N
  nu = r - div - 0.5 * sig^2
...
solveImplicitTridiagonal=function(V, pu, pm, pd, lambdaL, lambdaU, colI)
{
  ...
  # Testing the implicit method ----
print(paste("European Call Price=",Implicit2(isAmerican=FALSE,isCall=TRUE, K=100, Tm=1, S0=100, r=0.06, sig=0.2, N=3, div=0.03, dx=0.2)$Price))
print(paste("European Put Price=",Implicit2(isAmerican=FALSE,isCall=FALSE, K=100, Tm=1, S0=100, r=0.06, sig=0.2, N=3, div=0.03, dx=0.2)$Price))
#Implicit American Put = 4.9221 (verified from textbook)
#Implicit American Call=7.5608 (not verified)
#Implicit European Call=7.5608 (not verified)
#Implicit European Put =4.768 (not verified)
...
print(paste("European Call Price=",Implicit2(isAmerican=FALSE,isCall=TRUE, K=100, Tm=1, S0=100, r=0.06, sig=0.25, N=as.integer(c[4]), Nj=as.integer(c[2]), div=0.03, dx=as.double(c[3]))$Price))
```
...
QuestionE<-function(error,N=100,Tm,nsd,isAmerican, isCall, K, S0, r, sig, div, dx){
  ...
  repeat{
    ...
    print(N)
    explicit_price=Explicit2(isAmerican=FALSE,isCall=isCall, K=K, Tm=Tm, 
...
print(CrankNicholson(isAmerican=TRUE,isCall=FALSE, K=100, Tm=1, S0=100, r=0.06, 
               sig=0.25, N=1128, div=0.03, dx=0.2)$Price)
